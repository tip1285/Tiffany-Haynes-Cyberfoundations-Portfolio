...
```text
I expect to see random characters or gibberish that doesn't make any sense when inspecting encrypted data as bytes. Encrypted data should not look like the original report because encrypted data should not be readable. The original report has been transformed so that the message is kept confidential.
```
...
```text
The goal of encryption is confidentiality, and this lab demonstrated this by keeping the incident report secure and confidential. A passphrase was created to protect the incident report. Encryption alone does not prevent a file from being deleted or modified by someone with the right access.
```
...
```text
Encryption protects confidentiality — it does not prevent someone from deleting the file, stealing the whole encrypted blob, or tampering with it. Someone who has is authorized or has access to the file can copy or delete the file.
```

2. What job did the passphrase perform?

```text
The passphrase protects the private-key file. It's an optional secret that encrypts your private-key file on disk, so a stolen key file is not immediately usable. It is not the same thing as your account password which is used to authenticate the account.
```

3. Why does the match test prove correct recovery but not prove who handled the file?

```text
The match test proves the 
```
