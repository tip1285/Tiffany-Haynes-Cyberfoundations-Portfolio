...
```
I counted a total of 11 files. The name families I spotted were invoices, logs and notes.
```
...
```
02:47 DENIED badge 4471 storeroom door
I flagged this line due to the fact that an event was logged for a denied badge at 02:47 in the morning. The other 2 events were logged during what most businesses would consider normal business hours. I would need to know what the normal business hours are to determine if it's really a suspicious event.
```
...
```
Currently everyone (owner, group and others) has access to read and write the file.
```
...
```
When you run ls it gives you a listing of everything inside a directory or file. If you skipped running ls and went straight to acting, you could potentially copy the wrong file if you ran the cp command. If you ran the rm command without running ls first, you could potentially remove the wrong file, which can't be reversed.
```
...
```
In a real security job reducing six hundred lines down to three worth reading saves a lot of time. It would take too long to browse through all six hundred lines; therefore, run a pattern and see what it catches.
```
...
```
The command to run/find a file by its contents is to help you search for specific files that contain certain words, this narrows down the search. The actual filename may be completely different from the contents it holds, so searching by the contents can potentially help save time. The second command to audit the permissions of the file lets you know who has access to the file and what they can do with the file. One command helps you to actually find the file and the other command lets you know who can access it, so you need both to complete a workflow.
```
...
```
I think running the (grep command + request) scales up the furthest into professional work. Running this command pulls out everything with matching lines. If you're working with real systems that can narrow down the search tremendously.
```
