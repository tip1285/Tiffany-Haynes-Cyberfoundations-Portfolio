...
```text
Currently, the traffic from Grid Beacon (10.60.6.4) is being allowed because the security rule in place has the priority number of (300) which is the lowest number and the first rule that is being matched. If there is a new security rule being added with a lower priority (250) that states inbound traffic from 10.60.6.4 that uses TCP 8080 should be Denied; then, the traffic will now be denied. The lower the number the higher the priority.  
```
...
The result shows that the Grid Beacon was denied access which is what I predicted.
```
...
```text
I checked the VM and confirmed that it is still running. The original Allow is still present and correctly scoped. The temporary Deny is evaluated first due to the first match principle. Everything is healthy and running so the deny result was not due to a service not listening or a stopped VM.
```
...
```text
The correct Allow rule stopped working even though it was never edited, that was due to the first matching principle. The first matching principle finds the first matching instruction and executes it immediately. Rules are evaluated in order of priority, starting from the lowest numerical value. Since the correct Allow priority number was 300 and the new Deny rule priority number was 250, the 250 will match before 300; therefore, the 300 priority stops working. 
```
